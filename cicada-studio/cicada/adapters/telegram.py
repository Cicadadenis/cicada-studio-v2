...
class TelegramAdapter:
    ...
    def _send_media(self, method: str, field: str, chat_id: int, file, caption: str = "") -> dict:
        ...
        file может быть:
          ...
          - base64 data URL            → декодируется и отправляется multipart/form-data
        """
        import os
        ...
        if isinstance(file, str) and file.startswith("data:"):
            ...
            file_bytes = _base64.b64decode(encoded)
            ext_map = {
                "image/png": "png", "image/jpeg": "jpg", "image/jpg": "jpg",
                "image/gif": "gif", "image/webp": "webp", "image/bmp": "bmp",
                "application/pdf": "pdf",
            }
            ext = next((v for k, v in ext_map.items() if k in header), "bin")
            buf = BytesIO(file_bytes)
            buf.name = f"{field}.{ext}"
            resp = self._session.post(
                self.base + method,
                data={"chat_id": chat_id, "caption": caption},
                files={field: (buf.name, buf)},
                timeout=60,
            )
            resp.raise_for_status()
            return resp.json()
